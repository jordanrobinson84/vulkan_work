...
#define VK_DEVICE_FUNCTION(function) function = (PFN_##function)( vkGetDeviceProcAddr( device, #function )); assert( function != nullptr);

VulkanDevice::VulkanDevice(VulkanDriverInstance * __instance, uint32_t deviceNumber, bool debugPrint = true){
    instance = __instance;
...
// void VulkanDriverInstance::setupDevice(uint32_t deviceNumber, bool debugPrint){
    ...
    // if (debugPrint){
    //     uint32_t majorVersion = VK_VERSION_MAJOR(device.deviceProperties.apiVersion);
    //     uint32_t minorVersion = VK_VERSION_MINOR(device.deviceProperties.apiVersion);
    //     uint32_t patchVersion = VK_VERSION_PATCH(device.deviceProperties.apiVersion);
