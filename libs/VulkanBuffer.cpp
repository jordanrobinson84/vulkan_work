...
void VulkanImage::blitImage(VkCommandBuffer commandBuffer, VulkanImage& destImage, VkImageBlit *blitPtr, VkFilter filter){
    // Only attempt to blit if blits are permitted with the given images
    assert(deviceContext->getSupportedFormat({imageCreateInfo.format}, imageCreateInfo.tiling, VK_FORMAT_FEATURE_BLIT_SRC_BIT ) == imageCreateInfo.format);
    assert(deviceContext->getSupportedFormat({destImage.getImageCreateInfo().format}, destImage.getImageCreateInfo().tiling, VK_FORMAT_FEATURE_BLIT_DST_BIT ) == destImage.getImageCreateInfo().format);

    // Default blit parameters
    VkImageBlit defaultBlit;
    defaultBlit.srcSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1};
    defaultBlit.srcOffsets[0]  = { 0,0,0 };
    defaultBlit.srcOffsets[1]  = { (int32_t)imageCreateInfo.extent.width, (int32_t)imageCreateInfo.extent.height, 1};
    if(destImage.getImageCreateInfo().extent.width < defaultBlit.srcOffsets[1].x){
        defaultBlit.srcOffsets[1].x = destImage.getImageCreateInfo().extent.width;
    }
    if(destImage.getImageCreateInfo().extent.height < defaultBlit.srcOffsets[1].y){
        defaultBlit.srcOffsets[1].y = destImage.getImageCreateInfo().extent.height;
    }
    defaultBlit.dstSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1};
    defaultBlit.dstOffsets[0]  = { 0,0,0 };
    defaultBlit.dstOffsets[1]  = { (int32_t)imageCreateInfo.extent.width, (int32_t)imageCreateInfo.extent.height, 1};
    if(destImage.getImageCreateInfo().extent.width < defaultBlit.dstOffsets[1].x){
        defaultBlit.dstOffsets[1].x = destImage.getImageCreateInfo().extent.width;
    }
    if(destImage.getImageCreateInfo().extent.height < defaultBlit.dstOffsets[1].y){
        defaultBlit.dstOffsets[1].y = destImage.getImageCreateInfo().extent.height;
    }

    // If blit parameters aren't specified, use default
    if(blitPtr == nullptr){
        blitPtr = &defaultBlit;
    }
...
void VulkanImage::copyImageToBuffer(VkCommandBuffer commandBuffer, VulkanBuffer& destBuffer, VkBufferImageCopy *imageCopyPtr){
    ...
    defaultImageCopy.bufferRowLength                   = cImageCreateInfo.extent.width; 
    defaultImageCopy.bufferImageHeight                 = cImageCreateInfo.extent.height;
    defaultImageCopy.imageSubresource                  = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1};
    defaultImageCopy.imageOffset                       = {0, 0, 0};
    defaultImageCopy.imageExtent                       = cImageCreateInfo.extent;

    if(imageCopyPtr == nullptr){
        imageCopyPtr = &defaultImageCopy;
    }
...
void VulkanImage::saveImage(const std::string& imageFileName){
    ...
    VulkanImage destImage(deviceContext, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_TYPE_2D, blitDstFormat, imageCreateInfo.extent);
    destImage.createImageView(VK_IMAGE_VIEW_TYPE_2D, VK_IMAGE_ASPECT_COLOR_BIT);

    // Perform blit to an R8G8B8A8 Image for saving to an image file
    if((!isRGBA || bytesPerPixel != 4) && isBlittable){
        // Blit then copy
        deviceContext->vkBeginCommandBuffer(copyCommandBuffer[0], &cbBeginInfo);
        blitImage(copyCommandBuffer[0], destImage);
        destImage.copyImageToBuffer(copyCommandBuffer[0], imageBuffer);
        deviceContext->vkEndCommandBuffer(copyCommandBuffer[0]);
    }else{
        // Standard image copy
        deviceContext->vkBeginCommandBuffer(copyCommandBuffer[0], &cbBeginInfo);
        copyImageToBuffer(copyCommandBuffer[0], imageBuffer);
        deviceContext->vkEndCommandBuffer(copyCommandBuffer[0]);
    }
